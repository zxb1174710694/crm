<%@ page import="com.myCRM.Settings.domain.DicValue" %>
<%@ page import="java.util.List" %>
<%@ page import="java.util.Map" %>
<%@ page import="java.util.Set" %>
<%@ page import="com.myCRM.workbench.domain.Tran" %>
...
request.getServerPort()+request.getContextPath()+"/";
	...
	Set<String> set = map.keySet();	//key表示阶段，value表示可能性

	//通过可能性来截取阶段分界点：当可能性为0的时候，将下标取出
	int index = 0;
	for(int i = 0;i < stageList.size();i++){
		DicValue stage = stageList.get(i);
		String possible = map.get(stage.getValue());

		if ("0".equals(possible)){
			index = i;
			break;
		}
	}
...
<body>
	...
	<div style="position: relative; left: 40px; top: -50px;">
		...
		<%--
			分析：当可能性为0的时候，前面七个都是一样的状态；
			     当可能性为100的时候，后面两个状态是一样的；
		--%>
		<%
			//获取当前阶段状态（java脚本不能出现el表达式）
			Tran t = (Tran) request.getAttribute("t");
			String currentStage = t.getStage();
			String currentPossible = map.get(currentStage);

			System.out.println("当前页面阶段："+currentStage+",当前可能性："+currentPossible);

			if ("0".equals(currentPossible)){
				//说明前面七个都是一样的状态；
				for (int i = 0;i < stageList.size();i++){
					DicValue dicValue = stageList.get(i);
					String listStage = dicValue.getValue();
					String listPossible = map.get(listStage);

					//说明这是两个这的其中一个
					if ("0".equals(listPossible)){
						//如果与当前阶段名称一样的话，那就是红叉，否则黑叉
		...
		<%
				...
				for (int i = 0;i< stageList.size();i++){
					DicValue dt = stageList.get(i);

					String stage = dt.getValue();
		...
		<%
						...
						//当当前下标等于循环下标时，说明找到了当前的状态位置，所以给绿标
						if (i == index2){
							//绿色标记
	...
	<div style="position: relative; top: 0px;">
		...
		<div style="position: relative; left: 40px; height: 30px; top: 30px;">
			...
			<div style="width: 300px;position: relative; left: 650px; top: -60px;"><b>${t.possible}</b></div>
			<div style="height: 1px; width: 400px; background: #D5D5D5; position: relative; top: -60px;"></div>
			<div style="height: 1px; width: 400px; background: #D5D5D5; position: relative; top: -60px; left: 450px;"></div>
		</div>
