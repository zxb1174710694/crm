<%@ page import="com.myCRM.workbench.domain.Clue" %>
...
<script type="text/javascript">
	...
	$(function(){
		...
		$("#bundBtn").click(function () {
			...
			var parameter = "cid=${c.id}&"
			//获取勾选的文本框
			var $ch = $("input[name=ch]:checked");
			for (var i = 0;i < $ch.length;i++){
				parameter += "aid="+$($ch[i]).val();
	...
	function activity_clue() {
		$.ajax({
			url:"workbench/clue/activity_clue.do",
			data:{
				"id":"${c.id}"
			},
			type:"get",
			...
			success:function (data) {
				...
				$.each(data,function (i,n) {
					...
					html += '</tr>'
					/*这里的n表示的是activity，但是n.id的id是关系表的id
					* 这是因为三表联查，用关系表的id覆盖掉原来的，方便
					* 取消关联操作*/
				})

				//拼接数据
				$("#activity_clue").html(html);
			}
			...
			if (event.keyCode == 13){
				...
				$.ajax({
					...
					success:function (data) {
						...
						$.each(data,function (i,n) {
							html += '<tr>'
							html += '<td><input type="checkbox" value="'+n.id+'" name="ch"/></td>'
							html += '<td>'+n.name+'</td>'
							html += '<td>'+n.startDate+'</td>'
							html += '<td>'+n.endDate+'</td>'
							html += '<td>'+n.owner+'</td>'
	...
	function deleteRelation(id) {
		...
		var b = window.confirm("是否要解除关系？");

		if (b){
			$.ajax({
				url:"workbench/clue/delete_activity_clue.do",
				data:{
					"id":id
				},
				type:"post",
...
<body>
	...
	<div style="position: relative; left: 40px; top: -30px;">
		...
		<div style="position: relative; height: 50px; width: 500px;  top: -72px; left: 700px;">
			<button type="button" class="btn btn-default" onclick="window.location.href='workbench/clue/convert.jsp?id=${c.id}&aname=${c.fullname}${c.appellation}&company=${c.company}&owner=${c.owner}';"><span class="glyphicon glyphicon-retweet"></span> 转换</button>
